...
#+END_SRC
** matlab
- Start server with =M-x matlab-start=
- Stop server with =M-x matlab-server-kill=
- Open shell with =M-x matlab-shell=
- Run code by highlighting and pressing =C-c C-r= (=matlab-shell-run-region=)
